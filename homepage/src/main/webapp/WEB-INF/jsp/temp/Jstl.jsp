...
<ul>
	<c:choose>
		...
		<c:when test="${step eq '2'}">
			...
			<%-- <c:import url="/tmep/selectList.do" charEncoding="utf-8"/> --%>
			</li>
		</c:when>
		...
		<c:when test="${step eq '6'}">
			<li>
				...
				<h4> 기본문법 : <br/>
					&#60;c:forEach var="변수명" items="반복은 변수"&#62;<br/>
						반복문 내용<br/>
						&#60;c:forEach&#62;
				</h4>
		...
		<c:when test="${step eq '8'}">
			<li>
				...
				URL 2 : <c:url var="link2" value="/temp/jstImport.do">
							<c:param name = "test" value="테스트 호출"/>
						</c:url>
						<br/>
		...
		<c:when test="${step eq '15'}">
			<li>
				...
				<c:set var="string" value="Welcome to JSP Progaramming"/>
		...
		<c:when test="${step eq '20'}">
			<li>
				<h3>fmt: formatDate : 제공된 패턴 및 스타일을 사용하여 시간및 날짜를 형식화함 </h3>
